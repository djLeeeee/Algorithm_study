...
def main():
    ...
    while True:
        try:
            ...
            filename = f"2023/{date}/{userID}/[{platform}]_{idx}_{title}.py"
            os.makedirs(os.path.dirname(filename), exist_ok=True)
            with open(filename, "w") as f:
                f.write(f"# Generated by make_my_study_record.py\n")
                f.write(f"# BOJ LINK : https://www.acmicpc.net/problem/{idx}\n")
                f.write(f"# My submission : https://www.acmicpc.net/status?option-status-pid=on&problem_id={idx}&user_id={userID}&language_id=-1&result_id=-1&from_problem=1\n")
                f.close()
        except EOFError:
            break
